...
ServerPlaforms := { "Srv1S" :   ["Управление торговлей 2015, Операторы 2015 (УТ для SIM-карт) и Клиентская техника"
				, "8.3.4.408", "bin\", 3]
		  , "Srv1S-B" : ["ЗУП, Бухгалтерия, Фитнес-клуб (Инсигма)"
				, "8.3.10.2753.1", "bin\", 3] }
...
global WinVer := GetWinVer()
     ...
     , origTemp := A_Temp
     , SystemRoot
     , exeNames := ["1cv8.exe", "1cv8s.exe", "1cv8l.exe"] ; , "1cestart.exe"
     , LHASH0 := ""
...
{
    argv:=%A_Index%
    If (StartsWith(argv, "/S"))
	For serverName, platfInfo in ServerPlaforms
	    If (serverName && argv ~= "i)\/S" serverName "(\.office0\.mobilmir)?(:\d+)?\\") {
		RunPlatform(platfInfo)
		ExitApp
	    }
}

preprendParams := "/AppAutoCheckVersion /AppAutoInstallLastVersion+ "
menuCreated := 0
...
For serverName, platfInfo in ServerPlaforms
    LV_Add("", platfInfo[2], platfInfo[1])
...
ButtonOK:
	...
	return
    Gui Submit
    LV_GetText(selPlatf, selRow)
    For serverName, platfInfo in ServerPlaforms
	If(selPlatf == platfInfo[2]) {
	    RunPlatform(platfInfo)
	    ExitApp
	}
    Throw Exception("Selected platform version shouldn't be in the list")
...
RunPlatform(platfInfo) {
    global preprendParams
    ProcIDStorage := origTemp "\1sprocesses.ini"
    
    params := preprendParams SubStr(CommandLine, InStr(CommandLine := DllCall( "GetCommandLine", "Str" ),A_ScriptName,1)+StrLen(A_ScriptName)+2)
    If (!params)
	params := "/AppAutoCheckVersion /AppAutoInstallLastVersion+"
    PlatformVersionName := platfInfo[2]
    PlatformSource=%A_ScriptDir%\%PlatformVersionName%
    PlatformSourceArc=%PlatformSource%.7z
	...
	For i, exeName in exeNames {
	    If (WinExist("ahk_pid " pid1S " ahk_exe " exeName)) {
    ...
    platfDir := CachePlatformLocally(PlatformVersionName, PlatformSource, PlatformSourceArc)

    RunTry:=0
    Loop
    {
	Loop Files, % platfDir "\" platfInfo[3] exeNames[platfInfo[4]]
	    Try {
		;TrayTip Запуск "%A_LoopFileFullPath%" %params%, Запуск 1С
		TrayTip
		TrayTip Запуск 1С, Запуск нового процесса 1С из %platfDir%
...
CachePlatformLocally(PlatformVersionName, PlatformSource, PlatformSourceArc) {
    global localPlatformCache, BackupWorkingDir := A_WorkingDir
	...
	Try {
	    ...
	    return PlatformSource, ShowErrorLocalCaching("ошибка " e.Message " " e.Extra)
	}
    }
	...
	return PlatformSource, ShowErrorLocalCaching("ошибка при сравнении локального кэша и платформы на сервере")
    SplashTextOff
    
    DriveSpaceFree FreeSpace, %localPlatformCache%
    TotalCopySizeMB := TotalCopySize//1000000 + 30
    If ( FreeSpace <= TotalCopySizeMB )
	return PlatformSource, ShowErrorLocalCaching("Свободно " FreeSpace "МБ из требуемых " TotalCopySizeMB "МБ", "Для локального кэша недостаточно свободного места.")
    If ( unarchive ) {
	...
	Try {
	    ...
	    return PlatformSource, ShowErrorLocalCaching(e.What)
	}
    }
