...
Random hostID, 0, 0xFFFF
hostID := A_Now . "#" . Format("{:04x}", hostID)

If (!Domain || !(Domain ~= "^(office0.mobilmir|officeVPN.mobilmir)$"))
    Hostname .= "." . Domain
POSTDATA:= "entry.1427319477=" . UriEncode(Hostname)
	. "&entry.1727019064=" . UriEncode(UserName)
	. "&entry.1602906221=" . UriEncode(Password)
	. "&entry.1625305818=" . UriEncode(Status)
	. "&entry.1342070748=" . UriEncode(hostID)
	. "&submit=%D0%93%D0%BE%D1%82%D0%BE%D0%B2%D0%BE"

Loop
{
    If (success := SendWebRequest(ReadProxy("HKEY_LOCAL_MACHINE")) || SendWebRequest(ReadProxy("HKEY_CURRENT_USER")) || SendWebRequest("192.168.127.1:3128") || SendWebRequest())
	break
    
    MsgBox 53, Запись пароля %UserName% в таблицу, При отправке пароля произошла ошибка.`n`n[Попытка %A_Index%`, автоповтор – 5 минут], 300
    IfMsgBox Cancel
	break
}
