...
(
    ...
    IF NOT EXIST "%tgt%\AppData\Local\ACL-backup" MKDIR "%tgt%\AppData\Local\ACL-backup"
...
(
    ECHO Saving backup of ACL for "%tgt%"
    %SetACLexe% -on "%tgt%" -ot file -actn list -lst "f:sddl;w:d,o,g" -bckp "%tgt%\AppData\Local\ACL-backup\fullprofile.%now%.sddl"
    ...
    CALL "%~dp0..\CheckWinVer.cmd" 6.2 && %SystemRoot%\System32\TAKEOWN.exe /F "%tgt%" /A /R /D Y /SKIPSL >NUL
    IF ERRORLEVEL 1 %SystemRoot%\System32\TAKEOWN.exe /F "%tgt%" /A /R /D Y >NUL
    REM -rec cont_obj -actn setowner -ownr "n:%UIDAdministrators%"
    %SetACLexe% -on "%tgt%" -ot file -rec cont_obj -actn rstchldrn -rst dacl
    REM Allow users modify, do not allow execute
    %SetACLexe% -on "%tgt%" -ot file -actn setprot -op "dacl:p_nc;sacl:np" -actn clear -clr dacl -actn ace -ace "n:%UIDAdministrators%;p:full;i:sc,so" -actn ace -ace "n:%UIDSYSTEM%;p:full;i:sc,so" -actn ace -ace "n:%DirUserName%;p:change,FILE_DELETE_CHILD;i:sc" -actn ace -ace "n:%DirUserName%;p:write,read,FILE_DELETE_CHILD,DELETE;i:io,so"
    %SetACLexe% -on "%tgt%" -ot file -rec cont_obj -actn setowner -ownr "n:%DirUserName%"
    ...
    %SetACLexe% -on "%tgt%" -ot file -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
    %SetACLexe% -on "%tgt%\AppData\Local\Microsoft\Windows\UPPS" -ot file -rec cont_obj -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
    rem %SetACLexe% -on "%tgt%\AppData\Local\Microsoft\Windows\UPPS\UPPS.bin" -ot file -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
    %SetACLexe% -on "%tgt%\AppData\Local\TileDataLayer\Database\EDBtmp.log" -ot file -rec cont_obj -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
    %SetACLexe% -on "%tgt%\AppData\LocalLow" -ot file -rec cont_obj -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
    
    FOR %%A IN ("%tgt%\AppData\Local\Microsoft\Windows\UsrClass.dat*" "%tgt%\ntuser.dat*" "%tgt%\ntuser.ini") DO %SetACLexe% -on %%A -ot file -actn setowner -ownr "n:%UIDSYSTEM%" -actn setgroup -grp "n:%UIDSYSTEM%"
...
(
    ...
    %SetACLexe% -on %1 -ot file -rec cont_obj -actn list -lst "f:sddl;w:d,o,g" -bckp "%~2.%now%.sddl"
...
(
    ...
    %SetACLexe% -on %1 -ot file -actn restore -bckp "%restoreFile%" && DEL "%~2.flag"
    EXIT /B
:restoreACLUnflagSubdir <path> <backup-name>
(
    %SetACLexe% -on %1 -ot file -actn restore -bckp "%~2.%now%.sddl" && DEL "%~2.flag"
EXIT /B
)
...
(
    IF "%~2"=="" EXIT /B
    REM DACL(not_protected+auto_inherited):Все,FILE_LIST_DIRECTORY,deny,no_inheritance;Owner:СИСТЕМА;Group:СИСТЕМА
    %SetACLexe% -on "%2" -ot file -actn setprot -op "dacl:np" -actn ace -ace "n:%UIDEveryone%;p:FILE_LIST_DIRECTORY;i:np;m:deny" -actn setowner -ownr "n:%DirUserName%"
    SHIFT /2
