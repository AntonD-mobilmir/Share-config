...
If PendMoves
{
    FileAppend Pending moves detected. Won't do anything until reboot., *, cp1
...
{
    RepeatOnNextBoot=1
    FileAppend Cannot delete %ErrorLevel% files with mask "*.moz-delete". Scheduling move on reboot., *, cp1
    Loop *.moz-delete
...
Loop *.moz-upgrade
{
    ...
    {
	RepeatOnNextBoot=1
	FileAppend Cannot rename "%OutFileName%". Scheduling move on reboot., *, cp1
	DllCall("MoveFileEx", "Str", A_LoopFileLongPath, "Str", "", "UInt", 0x4) ; 0x4 = MOVEFILE_DELAY_UNTIL_REBOOT
    }
}
