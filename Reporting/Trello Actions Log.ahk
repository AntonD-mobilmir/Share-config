...
Loop
{
	...
	If (actionsToLog.HasKey(actn.type)) {
	    ...
	    If (!curLn)
		continue
	    out .= actn.date "`t" curLn "`thttps://trello.com/c/" actn.data.card.shortLink "`n"
	}
    }
    flog.WriteLine(query " → " lastresp)
} Until !maxIndex
flog.Close()
reportName = %A_TEMP%\%A_ScriptName%-%A_Now%.txt
...
updateCard(data) {
    ...
    If (out)
	return data.board.name ": " data.card.name "`t" SubStr(out, 1, -2)
